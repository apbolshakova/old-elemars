...
if (!bestScore.score) document.querySelector('.last_best_score').innerHTML = 'Рекорд: пока никто не сохранял. Стань первым!';
else {
    document.querySelector('.last_best_score').innerHTML = 'Рекорд: ' + bestScore.score + ' очков. Игрока звали ' + (bestScore.name || 'никак') + '.';
}
    ...
    this.draw = function () {
        ...
        clouds.elements.forEach((cloud, cloudNum) => {
            ...
            switch (Math.floor(cloudNum / cloudsPerScreen)) {
                case 0:
                    ctx.drawImage(cloudImg, -1 * canvas.width + clouds.x + cloud.x, cloud.y);
                    break;
                case 1:
        ...
        backForest.elements.forEach((forestEl, forestElNum) => {
            ...
            switch (Math.floor(forestElNum / backForestPerScreen)) {
                case 0:
                    ctx.drawImage(elImg, -1 * canvas.width + backForest.x + forestEl.x, forestEl.y, elImg.width * 0.75, elImg.height * 0.75);
                    break;
                case 1:
                    ctx.drawImage(elImg, backForest.x + forestEl.x, forestEl.y, elImg.width * 0.75, elImg.height * 0.75);
                    break;
                case 2:
                    ctx.drawImage(elImg, 1 * canvas.width + backForest.x + forestEl.x, forestEl.y, elImg.width * 0.75, elImg.height * 0.75);
            }
        ...
        for (let i = -1; i <= 1; i++) {
            ctx.drawImage(assetLoader.imgs.grass, i * gameWidth + backGrass.x, backGrass.y, assetLoader.imgs.grass.width, assetLoader.imgs.grass.height * 2);
        }
        ...
        frontForest.elements.forEach((forestEl, forestElNum) => {
            ...
            switch (Math.floor(forestElNum / frontForestPerScreen)) {
                case 0:
                    ctx.drawImage(elImg, -1 * frontForestWidth + frontForest.x + forestEl.x, forestEl.y);
                    break;
                case 1:
                    ctx.drawImage(elImg, frontForest.x + forestEl.x, forestEl.y);
                    break;
                case 2:
                    ctx.drawImage(elImg, frontForestWidth + frontForest.x + forestEl.x, forestEl.y);
            }
        });
...
function createPlayer(player) {
    ...
    if (selectedCharacter === 'ice') {
        player.walkSheet = new SpriteSheet(assetLoader.imgs.iceRun.src, player.width, player.height);
        player.jumpSheet = new SpriteSheet(assetLoader.imgs.iceJump.src, player.width, player.height);
    }
    if (selectedCharacter === 'fire') {
        player.walkSheet = new SpriteSheet(assetLoader.imgs.fireRun.src, player.width, player.height);
        player.jumpSheet = new SpriteSheet(assetLoader.imgs.fireJump.src, player.width, player.height);
    }
            ...
            switch (type) {
                case 0:
                    obstacles.elements.push(new Sprite(
                        rand(500, frontForestWidth - 619 - 834) + frontForestWidth * i,
                        560,
                        619,
                        452,
                        'brokenTree'));
                ...
                case 1:
                    obstacles.elements.push(new Sprite(
                        rand(500, frontForestWidth - 492 - 834) + frontForestWidth * i,
                        680,
                        492,
                        394,
                        'log'));
                ...
                default:
                    if (level > 2) {
                        obstacles.elements.push(new Sprite(
                            rand(2000, frontForestWidth - 227 - 834) + frontForestWidth * i,
                            -3400,
                            227,
                            614,
                            'meteorite',
...
function mainMenu() {
    ...
    document.querySelectorAll('.back').forEach((el) => el.onclick = function () {
    ...
    document.querySelector('#show-instructions-btn').onclick = function () {
        document.querySelector('#main').style.display = 'none';
    ...
    document.querySelectorAll('.level_button').forEach((el) => el.onclick = function () {
        document.querySelectorAll('.level_button').forEach((el) => el.classList.remove('button_active'));
        this.classList.add('button_active');
    ...
    document.querySelector('#download-result').onclick = function () {
        ...
        if (!bestScore.score) {
            ...
            text = (bestScore.name ? bestScore.name : 'Ты') + ' молодец и набрал ' + bestScore.score
                + ' очков, играя за ' + (bestScore.character === 'ice' ? 'Лёд' : 'Огонь') + '!';
        }
...
function startGame() {
    ...
    frontObstacles.x = -frontForestWidth;
    for (let i = 0; i < 3; i++) {
        obstacles.elements.push(new Sprite(3840, 860, 834, 220, 'water'));
        for (let i = 0; i < 1; i++) {
            obstacles.elements.push(new Sprite());
